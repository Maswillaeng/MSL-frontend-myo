...
const Board = () => {
    ...
    return (
        <div className='w-full h-screen items-center justify-center bg-[#FBF9EC] bg-cover'>
            <div>
                {/* 상단 이미지 */}
                <div className='w-full h-28' style={{ backgroundImage: 'url(https://cdn.pixabay.com/photo/2015/03/30/12/35/mojito-698499_1280.jpg)'}}>
                </div>

                <div className="w-full h-20">
                    <ul className="m-auto flex justify-around pt-6 cursor-pointer">
                        { board_tab.map(item =>
                            <li className={ index === item.id ? "font-bold" : "" } key={ item.id}
                ...
                <div>
                    ...
                    <section className="container px-4 mx-auto">
                        ...
                        <div className="flex items-center justify-between mt-6">
                            ...
                            : board_tab.filter(item => index === item.id).map((item, index) =>
                                <div key={index}>{ item.tab }</div>)
                    }
                </div>
            </div>
        </div>
