    ...
    const LikeMatch = async () => {
      try {
        const responses = await Promise.all(
          postIds.map((item) => axios.get(`api/post/posts/${item}`))
        );
  ...
  return (
    <>
      ...
      <span className="m-auto">
        총
        <span className="text-red-500 text-lg font-bold">{totalPostCount}</span>
        개 글이 있습니다
      </span>
        ...
        <div className="-mx-4 -my-2 overflow-x-auto sm:-mx-6 lg:-mx-8">
          <div className="inline-block min-w-full py-2 align-middle md:px-6 lg:px-8">
            <div className="overflow-hidden border border-gray-200 dark:border-gray-700 md:rounded-lg p-2">
              ...
              {loading && (
                <div className="text-center text-3xl mt-5"> 로딩 중... </div>
              )}
              ...
              <div className="mx-1 grid grid-cols-4">
                {allList.map((item, idx) => (
                  <div className="text-center p-5 h-auto" key={item.id}>
                    ...
                    {/* 조회수는 아직 구현 중 */}
                    <span className="text-sm pl-3">👀 9999</span>
                    {/* 좋아요 제작 중 */}
